...
coding errors (file scopenonly at present). Its advantage over the compiler is that it checks every
line rather than bailing out after finding the first error. 
It can be very useful innporting Perl code to Raku, and it may eventually be using
